...
const CheackoutForm = ({bookingData}) => {
  ...
  const [processing, setProcessing] = useState(false)
    ...
    const {price, buyerName, email, _id, brandName} = bookingData

    useEffect(() => {
      // Create PaymentIntent as soon as the page loads
      fetch("https://resale-assignment-server.vercel.app/create-payment-intent", {
        method: "POST",
        headers: { 
    ...
    const handleSubmit = async (event) => {
        ...
        if(paymentIntent.status === "succeeded"){
          ...
          fetch('https://resale-assignment-server.vercel.app/payments', {
            method: 'POST',
            headers: {
              'content-type': 'application/json',
              authorization: `bearer ${localStorage.getItem('resaleToken')}`
