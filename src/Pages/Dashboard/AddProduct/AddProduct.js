...
const AddProduct = () => {
  ...
  const handleProductAdd = (event) => {
    ...
    fetch("https://resale-assignment-server.vercel.app/products", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="mx-4 p-4">
      ...
      <form onSubmit={handleProductAdd} className="grid grid-cols-1 gap-5">
        ...
        <select
          name="category"
          className="select select-success w-full max-w-sm mx-auto"
          required
        >
        ...
        <input
          ...
          required
        />
        <label htmlFor="img" className="block text-center font-bold">
          Product Photo:
        </label>
        <input
          type="file"
          name="img"
          className="file-input file-input-bordered file-input-success w-full max-w-sm mx-auto"
          required
        />
        <textarea
          name="desc"
          required
          className="textarea textarea-success max-w-sm mx-auto w-full"
          placeholder="Product Description"
        ></textarea>
        <input
          type="submit"
          className="bg-green-400 hover:bg-green-500 text-black max-w-sm mx-auto p-3 rounded-lg w-full"
